...
- active 重启后，allocator 会对当前 pair 的 `error` assignment 自动尝试一次 `error -> pending` 恢复
- 运维也可以通过 `warehouse ha assignments drain/release/retry` 显式推进 `draining -> released` 或 `error -> pending`
...
- active 是唯一对外写入的主实例
- standby 是只接 internal 同步流量的被动副本
- PostgreSQL 不只存业务元数据，也承担“共享控制面”
- active 先把本地文件写成功，再把变化记到 outbox
- standby 通过“历史补齐 + 后续增量”维持第二份本地数据
- assignment / generation / fence 负责保证“当前到底谁能往哪个 standby 写”

如果只记一件事，就是：

- `cluster_nodes` 解决“谁在线、地址是什么”
- `cluster_replication_assignments` 解决“谁跟谁复制、当前在哪一代”
- `replication_outbox` / `replication_offsets` 解决“复制到哪里了”
...
- 当前状态含义如下：
  ...
  - `error`
    ...
    - 它也不是有效 assignment
    - 当前需要运维执行 `retry` 或等待 active 启动期的一次自动恢复
...
- 它会根据健康 standby、当前有效 assignment 来决定：
  - 继续续租谁
  - 是否释放旧关系
  - 启动期是否尝试把 `error -> pending`
- 当前阶段它还是单 active -> 单 standby 模型的分配器，不是多副本调度器

#### 静态 peer 配置

- 当前代码已经移除 `peer_node_id` / `peer_base_url`
- “复制给谁”统一由 effective assignment 决定
- “地址是什么”统一由 `cluster_nodes.advertise_url` 提供
- 运维如果要显式访问某个实例，可以在 CLI 里使用 `--base-url`，但这只是一次性操作参数，不再是长期配置状态
...
2. 它不再是有效 assignment
3. 运维执行 `retry`，或者 active 启动时自动恢复一次 `error -> pending`
4. generation 进入新的一代
...
- assignment 已经具备 `pending -> reconciling -> replicating -> draining -> released` 主链路，以及失败进入 `error`、运维 `retry -> pending` 的恢复路径，但还没有完整 failover / fencing 状态机
...
6. 待实现：outbox 从“单 target”演进为“按 assignment fan-out”
7. 已完成：`warehouse ha assignments drain/release/retry` 可显式推进 assignment 状态迁移
8. 待实现：`warehouse ha assignments ...` 继续扩展到 rebalance 等观测与编排能力
9. 待实现：多 standby 编排、rebalance、drain、promote 自动化
...
- [x] allocator 会优先保持当前健康 assignment；如果没有有效 assignment，再选择最新健康心跳的 standby
...
- assignment 进入 `error` 后不会再被视为有效复制目标；active 重启时会自动尝试一次恢复，或者由运维执行 `warehouse ha assignments retry` 显式回到 `pending`
...
- 当前 active 会优先保留当前健康 assignment；如果当前 assignment 不可用，再选择最新健康心跳的一个 standby
